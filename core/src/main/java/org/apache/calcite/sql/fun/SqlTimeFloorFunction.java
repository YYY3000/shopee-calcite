 ...
 */
public class SqlTimeFloorFunction extends SqlMonotonicUnaryFunction {
  //~ Constructors -----------------------------------------------------------

  public SqlTimeFloorFunction(SqlKind kind) {
    super(kind.name(), kind, ReturnTypes.ARG0_OR_EXACT_NO_SCALE, null,
        OperandTypes.sequence(
            "'" + kind + "(<DATE> TO <TIME_UNIT>, <TIME_ZONE>)'\n"
                + "'" + kind + "(<TIME> TO <TIME_UNIT>, <TIME_ZONE>)'\n"
                + "'" + kind + "(<TIMESTAMP> TO <TIME_UNIT>, <TIME_ZONE>)'",
            OperandTypes.DATETIME,
            OperandTypes.ANY,
            OperandTypes.STRING),
        SqlFunctionCategory.NUMERIC);
    Preconditions.checkArgument(kind == SqlKind.TIME_FLOOR);
  }
  ...
  @Override public void unparse(SqlWriter writer, SqlCall call, int leftPrec,
    ...
    if (call.operandCount() == 3) {
      ...
      writer.sep("TO");
      call.operand(1).unparse(writer, 100, 200);
      writer.sep(",");
      call.operand(2).unparse(writer, 200, 300);
    }
